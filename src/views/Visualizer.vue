        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-4">
            ...
            <button @click="logout" class="logout-btn ml-2 px-5 py-2 bg-white text-gray-800 text-base rounded-full font-semibold border border-gray-300 shadow-sm hover:bg-gray-100 hover:text-black transition-all flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a2 2 0 01-2 2H7a2 2 0 01-2-2V7a2 2 0 012-2h4a2 2 0 012 2v1" /></svg>
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
      <div class="grid lg:grid-cols-4 gap-6">
        <div class="lg:col-span-1">
          <div class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 max-h-[600px] overflow-hidden">
            ...
            <div class="space-y-2 overflow-y-auto max-h-80 pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
              <div
...
const animationSpeed = ref(0.5) // default at 0.5
...
if(!urlParams.has("accessToken") && !localStorage.getItem("accessToken")) {
 window.location.href = "https://www.strava.com/oauth/mobile/authorize?client_id=127435&response_type=code&approval_prompt=auto&scope=read,activity:read,activity:read_all,read_all,profile:read_all&redirect_uri=https://athenaapi.vercel.app/callback"
}
activities.value = await fetch("https://athenaapi.vercel.app/data?accessToken="+urlParams.get("accessToken")).then(r=>r.json())
...
const startAnimation = () => {
  ...
  // Clear existing animated elements
...
const animateRoute = () => {
  ...
  const duration = (routeCoordinates.value.length / animationSpeed.value) * 50 // 50ms per point at 1x speed
...
const updateAnimatedPolyline = (pointsToShow) => {
  ...
  // Remove previous animated polyline
  if (animatedPolyline.value) {
    map.value.removeLayer(animatedPolyline.value)
  }
  
  // Create new animated polyline
...
const initializeMap = async () => {
  try {
    ...
    // Select first activity with polyline
...
const selectActivity = async (activity) => {
  try {
    ...
    // Decode polyline with enhanced decoder
    ...
    // Validate coordinate bounds (should be around Malaysia)
    ...
    // Create full polyline (initially visible)
